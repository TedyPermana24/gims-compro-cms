...
function WhatsAppPopup({ phone, email, teksGreeting, labelTombol, namaPerusahaan }: WhatsAppPopupProps) {
    ...
    <div ref={ref} className="relative">
        ...
        <div className="w-[300px] bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            <div className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:bg-[#25d366]/5 hover:border-[#25d366]/20 transition-all group">
              ...
              <a
                href={getWhatsAppHref(phone, teksGreeting)}
                target="_blank"
          ...
          <div className="bg-slate-50 px-4 py-2.5 border-t border-slate-100">
            <p className="text-[10px] text-slate-400 text-center">{namaPerusahaan}</p>
          </div>
        </div>
      </div>
      ...
      >
        <p className="text-sm font-semibold text-navy-dark flex items-center gap-2 group-hover:text-[#25d366] transition-colors">
          ...
          <span className="hidden md:block">{labelTombol}</span>
        </p>
      </button>
    </div>
  );
...
export default function FloatingWidgets() {
  ...
  const wa: GeneralSetting["whatsapp"] = generalData?.whatsapp ?? undefined;
  const phone = formatContactPhone(wa?.nomor ?? DEFAULT_CONTACT_PHONE);
  const email = wa?.email ?? DEFAULT_CONTACT_EMAIL;
  const teksGreeting = wa?.teksGreeting ?? "Halo GiMS! Saya ingin bertanya mengenai layanan Anda.";
  const labelTombol = wa?.labelTombol ?? "Apa yang bisa saya Bantu?";
  const namaPerusahaan = wa?.namaPerusahaan ?? "PT. Global Inovasi Mitra Solusi";
